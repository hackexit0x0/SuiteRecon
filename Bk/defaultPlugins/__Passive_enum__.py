...
def findomain(domain, output_file):
    ...
    try:
        ...
        command = ["findomain", "-t", domain, "-o", "-q"]
        subprocess.run(command, check=True)

        # Move findomain default output to the desired location
        default_output = f"{domain}.txt"
        if os.path.exists(default_output):
            os.rename(default_output, output_file)
            print(f"[+] Findomain results saved at: {output_file}")
        else:
            print(f"[!] No output file generated by findomain.")
    except FileNotFoundError:
...
def main(domain=None):
    try:
        if not domain:
            domain = input("[+] Enter the domain for enumeration: ").strip()
