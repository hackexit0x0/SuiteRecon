...
def nikto(domain):
    ...
    # Run the nikto command and save the output to a file
    command = f"nikto -ask=no -Tuning=x4567890ac -nointeractive -host {domain} | tee nikto.txt"
    os.system(command)
    print(f"[+] nikto.txt saved at {path}/nikto.txt")
