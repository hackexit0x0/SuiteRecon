...
def Sub_Subdomain(domain, output_list):
    """Process subdomain enumeration results."""
    active_path = f"results/{domain}/fetchSubdomains/Sub.Subfinder/"
    create_directory(active_path)

    files_to_read = ["subfinder.txt"]
    print("[+] Reading subdomain enumeration results:")
    for file_name in files_to_read:
        file_path = os.path.join(active_path, file_name)
        content = read_file(file_path)
        if content:
...
def main(domain):
    ...
    Sub_Subdomain(domain, output_list)
